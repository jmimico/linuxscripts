...
wget -q http://cdimage.debian.org/cdimage/openstack/current-8/SHA256SUMS -O ${PTH}checksums_DEBIAN_8
wget -q http://cdimage.debian.org/cdimage/openstack/current-9/SHA256SUMS -O ${PTH}checksums_DEBIAN_9
...
OCS_U14=`grep "ubuntu-14.04-server-cloudimg-amd64-uefi1.img$" ${PTH}checksums_UBUNTU_14 | awk '{print $1}'`
OCS_U16=`grep "ubuntu-16.04-server-cloudimg-amd64-uefi1.img$" ${PTH}checksums_UBUNTU_16 | awk '{print $1}'`
...
OCS_D9=`egrep "debian.*amd.*.qcow2$" ${PTH}checksums_DEBIAN_9 | awk '{print $1}'`
D9_FILE=`egrep "debian.*amd.*.qcow2$" ${PTH}checksums_DEBIAN_9 | awk '{print $2}'`
...
UBUNTU_14="https://cloud-images.ubuntu.com/releases/14.04/release/ubuntu-14.04-server-cloudimg-amd64-uefi1.img"
#UBUNTU_16="https://cloud-images.ubuntu.com/releases/16.04/release/ubuntu-16.04-server-cloudimg-amd64-disk1.img"
UBUNTU_16="https://cloud-images.ubuntu.com/releases/16.04/release/ubuntu-16.04-server-cloudimg-amd64-uefi1.img"
CENTOS_7="http://cloud.centos.org/centos/7/images/CentOS-7-x86_64-GenericCloud.qcow2"
DEBIAN_8="http://cdimage.debian.org/cdimage/openstack/current-8/${D8_FILE}"
DEBIAN_9="http://cdimage.debian.org/cdimage/openstack/current-9/${D9_FILE}"

# Get checksums from current openstack images
CS_U14=`openstack image show -c properties -f value "Ubuntu 14.04 - latest" | awk -F'=' '{print $5}'| awk -F"'" '{print $2}'`
CS_U16=`openstack image show -c properties -f value "Ubuntu 16.04 - latest" | awk -F'=' '{print $5}'| awk -F"'" '{print $2}'`
CS_C7=`openstack image show -c properties -f value "CentOS 7 - latest" | awk -F'=' '{print $5}'| awk -F"'" '{print $2}'`
CS_D8=`openstack image show -c properties -f value "Debian 8 - latest" | awk -F'=' '{print $5}'| awk -F"'" '{print $2}'`
CS_D9=`openstack image show -c properties -f value "Debian 9 - latest" | awk -F'=' '{print $5}'| awk -F"'" '{print $2}'`
...
if [ ${CS_U14} = ${OCS_U14} ]
	...
	EMAIL=yes
		if [ "${UPDATE}" = "--update" ]
			...
			LCS_U14=`sha256sum ${PTH}ubuntu-14.04-server-cloudimg-amd64-uefi1.img | awk '{print $1}'`
			...
			openstack image create --container-format bare --disk-format qcow2 --file ${PTH}ubuntu-14.04-server-cloudimg-amd64-uefi1.img --property description='Default user is "ubuntu" and access is only allowed using ssh keys.' --property hw_vif_multiqueue_enabled='true' --property sha256=${LCS_U14} --public "Ubuntu 14.04 - latest.tmp"
...
if [ ${CS_U16} = ${OCS_U16} ]
	...
	if [ "${UPDATE}" = "--update" ]
		...
		wget -q ${UBUNTU_16} -P ${PTH}
        	LCS_U16=`sha256sum ${PTH}ubuntu-16.04-server-cloudimg-amd64-uefi1.img | awk '{print $1}'`
		...
		openstack image create --container-format bare --disk-format qcow2 --file ${PTH}ubuntu-16.04-server-cloudimg-amd64-uefi1.img --property description='Default user is "ubuntu" and access is only allowed using ssh keys.' --property hw_vif_multiqueue_enabled='true' --property sha256=${LCS_U16} --public "Ubuntu 16.04 - latest.tmp"
...
if [ ${CS_C7} = ${OCS_C7} ]
	...
	if [ "${UPDATE}" = "--update" ]
		...
		openstack image create --container-format bare --disk-format qcow2 --file ${PTH}CentOS-7-x86_64-GenericCloud.qcow2 --property description='Default user is "centos" and access is only allowed using ssh keys.' --property hw_vif_multiqueue_enabled='true' --property sha256=${LCS_C7} --public "CentOS 7 - latest.tmp"
...
if [ ${CS_D8} = ${OCS_D8} ]
	...
	if [ "${UPDATE}" = "--update" ]
		...
		openstack image create --container-format bare --disk-format qcow2 --file ${PTH}${D8_FILE} --property description='Default user is "debian" and access is only allowed using ssh keys.' --property hw_vif_multiqueue_enabled='true' --property sha256=${LCS_D8} --public "Debian 8 - latest.tmp"
...
# Debian 9
if [ ${CS_D9} = ${OCS_D9} ]
	then echo "Debian 9 online checksum matches local, nothing to do"
	else
	echo "Debian 9 Update available. Run with --update" && echo "Debian 9 is out of date" >> ${PTH}${REPORT}
	EMAIL=yes
	if [ "${UPDATE}" = "--update" ]
                then
		echo "Debian 9 online checksum does not match local, downloading new image"
		wget -q ${DEBIAN_9} -P ${PTH}
		LCS_D9=`sha256sum ${PTH}${D9_FILE} | awk '{print $1}'`
                if [ ${OCS_D9} = ${LCS_D9} ] # Validate downloaded file
                        then echo "Debian 9 local checksums match, creating new glance image"
                        else echo "Debian 9 local checksum [${LCS_D9}] does not match online checksum [${OCS_D9}], please investigate"
			exit 1
                fi
		openstack image create --container-format bare --disk-format qcow2 --file ${PTH}${D9_FILE} --property description='Default user is "debian" and access is only allowed using ssh keys.' --property hw_vif_multiqueue_enabled='true' --property sha256=${LCS_D9} --public "Debian 9 - latest.tmp"
        	openstack image delete "Debian 9 - latest" && openstack image set "Debian 9 - latest.tmp" --name "Debian 9 - latest"
	fi
fi
